...
jest.mock('../../nodebackend/build/clients/vaultClient', () => ({
    login: jest.fn(),
    getSecret: jest.fn()
}));
jest.mock('../../nodebackend/build/clients/redisClient', () => ({
...
describe('POST /', () => {
    it('should respond with a status code of 200 for successful requests', async () => {
        ...
        const { getSecret } = require('../../nodebackend/build/clients/vaultClient');
    ...
    it('should respond with 500 if the Vault server is unavailable', async () => {
        ...
        const { login, getSecret } = require('../../nodebackend/build/clients/vaultClient');
