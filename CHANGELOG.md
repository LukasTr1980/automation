...
### Changed
- Frontend (Villa Anna Home): Show today's next irrigation time as a relative German label with "Uhr" for better readability.
...
### Changed
- Tooling: Updated backend and frontend dependency lockfiles after the latest `npm update` refresh, including `@lukastr1980/davis` 1.0.5, `axios` 1.16.0, TypeScript ESLint 8.59.2, and frontend React Query/Router patch releases.
- Backend (Dependencies): Refreshed `uuid` from 13.0.0 to 13.0.2 in the backend lockfile without changing source code.
...
### Changed
- Tooling: Updated backend and frontend dependencies after the latest `npm update` refresh (lockfiles only).
...
### Changed
- Tooling: Updated backend and frontend dependencies (lockfiles only).
...
### Fixed
- Backend (Tuya Bridge): Wait for confirmed Tuya device state changes before publishing MQTT switch feedback so the irrigation UI no longer flips back to stale values after a toggle.
- Backend (MQTT): Ignore retained irrigation `.../set` messages on subscribe to avoid replaying stale switch commands after reconnects.
...
### Fixed
- Backend (ET₀): Cloud cover query now uses QuestDB `dateadd` interval syntax so weekly calculations no longer fail on invalid interval errors.
